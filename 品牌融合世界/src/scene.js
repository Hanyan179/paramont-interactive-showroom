...
export class BrandScene {
 constructor(canvas,onSelect,onReady,media={},options={}){
  ...
  this.scene=new T.Scene();this.scene.background=new T.Color(atrium?'#071725':'#071b30');if(atrium){this.sky=createFlowSky(T,this.scene,q.exposure);this.scene.background=null;}this.scene.fog=new T.FogExp2(atrium?'#071725':'#071b30',.027);this.camera=new T.PerspectiveCamera(34,1,.1,100);this.camera.position.set(0,3.7,16);this.camera.lookAt(0,.15,0);
 ...
 tick(now,dt){if(!this.running)return;this.elapsed??=0;const elapsed=this.elapsed;this.elapsed+=dt;const factor=1-Math.exp(-dt*5);const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
  ...
  if(!document.hidden&&!this.suspended){this.sky?.update(elapsed,this.camera.aspect,reduced,0,0,0,0,this.canvas.height);this.renderer.info.reset();this.composer.render();this.renderFrames=(this.renderFrames||0)+1;}
 }
