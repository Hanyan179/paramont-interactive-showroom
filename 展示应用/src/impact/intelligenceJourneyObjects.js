...
import {proposalBrands,prepareBrandTexture,brandSurfaceRatio} from './intelligenceBrands.js';
...
export function createJourneyObjects(manager,quality){
 ...
 function refreshProduct(){
  if(disposed)return;
  if(!branded)branded=brandProductSurface(productTexture,brandTextures[4]);
 ...
 const brandTextures=proposalBrands.map(brand=>{
  const texture=loader.load(`/media/brand/${brand.logo}`,map=>{if(disposed)return;prepareBrandTexture(map,brand);refreshProduct();});
  texture.colorSpace=THREE.SRGBColorSpace;texture.anisotropy=quality.render.anisotropy;texture.userData.brandId=brand.id;return texture;
 });
 function addBrandMark(parent,index,width,x,y){
  const mark=texturePlane(brandTextures[index],width,width/brandSurfaceRatio,`${parent.name}-brand`);
  mark.position.set(x,y,.012);mark.renderOrder=parent.renderOrder+1;mark.material.side=THREE.FrontSide;
  mark.userData.brandId=proposalBrands[index].id;parent.userData.brandMark=mark;parent.add(mark);return mark;
 }
 ...
 const cards=Array.from({length:5},(_,i)=>{
  const card=texturePlane(cardTexture(i),2.65,1.86,`analysis-card-${i}`);card.renderOrder=3;addBrandMark(card,i,.66,.92,.815);
 ...
 const directionModels=cards.slice(0,4).map((card,i)=>{const model=createDirectionModel(i,brandTextures[i]);const positions=[[-.72,-.26,.20],[.80,-.42,.20],[.82,-.20,.20],[-.49,-.08,.20]];model.root.position.fromArray(positions[i]);model.root.scale.setScalar(i===1?.58:i===3?.67:.70);card.add(model.root);return model;});
 ...
 const commerceBrand=addBrandMark(details,4,.88,1.12,1.56);clipToPage(commerceBrand,details.userData.pageClip);
