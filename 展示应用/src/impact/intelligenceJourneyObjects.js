...
import {canvasTexture,label,rounded,cardTexture,screenTexture,reviewTexture,commerceTexture,commerceDetails,purchaseReviewTexture,designBoardTexture,reportTexture,translate} from './intelligenceSurfaces.js';
...
export function createJourneyObjects(manager,quality){
 ...
 const fragmentPairs=[['Repeat purchase','再次购买'],['A thoughtful gift','一份用心的礼物'],['Everyday favourite','日常最爱'],['Easy to use','使用方便'],['Worth recommending','值得推荐'],['Feels considered','细节很用心'],['Satin finish','缎光质感'],['Refined packaging','精致包装'],['Comfortable touch','舒适触感'],['Compact format','便携形态'],['Quiet elegance','含蓄优雅'],['A modern essential','现代日常之选'],['Growing interest','关注度上升'],['Seasonal demand','季节需求'],['New discovery','新的发现'],['Shopping intent','购买意愿'],['Collection detail','系列详情'],['Product comparison','商品比较'],['Champagne tone','香槟色调'],['Soft rose','柔和玫瑰'],['Warm neutrals','温暖中性色'],['Balanced palette','协调配色'],['Light and shade','明暗层次'],['Colour harmony','色彩协调'],['Daily routine','日常场景'],['Weekend plans','周末出行'],['Gift occasion','礼赠场景'],['Travel companion','旅行随身'],['Personal style','个人风格'],['Evening edit','晚间精选'],['Fine texture','细腻质地'],['Careful finishing','精细工艺'],['Consistent quality','稳定品质'],['Soft-touch finish','柔和触感'],['Considered form','精致形态'],['Customer confidence','用户信任']];
 ...
 const cards=Array.from({length:5},(_,i)=>{const mesh=texturePlane(cardTexture(i),2.65,1.86,`analysis-card-${i}`);const analysis=texturePlane(cardTexture(i,true),2.65,1.86,`analysis-content-${i}`);mesh.renderOrder=3;analysis.renderOrder=4;analysis.position.z=.008;analysis.userData.reveal={value:1};analysis.material.onBeforeCompile=shader=>{shader.uniforms.journeyReveal=analysis.userData.reveal;shader.fragmentShader='uniform float journeyReveal;\n'+shader.fragmentShader.replace('#include <map_fragment>','#include <map_fragment>\nif(vMapUv.x>journeyReveal) discard;');};analysis.material.customProgramCacheKey=()=> 'journey-reveal';mesh.add(analysis);const depth=new THREE.Mesh(new RoundedBoxGeometry(2.68,1.89,.10,2,.045),new THREE.MeshPhysicalMaterial({color:'#122438',metalness:.38,roughness:.22,transparent:true,depthWrite:false}));depth.position.z=-.075;depth.name='decision-card-depth';mesh.add(depth);if(i===2){const bars=new THREE.Group();bars.name='forecast-relief';[.30,.43,.39,.57,.72,.90].forEach((height,j)=>{const bar=new THREE.Mesh(new RoundedBoxGeometry(.20,1,.13,2,.025),new THREE.MeshPhysicalMaterial({color:j<3?'#699ebc':'#beddea',metalness:.22,roughness:.24,transparent:true,depthWrite:false}));bar.position.set(-1.03+j*.40,-.55+height/2,.10);bar.scale.y=height;bar.userData.height=height;bar.renderOrder=8;bars.add(bar);});mesh.add(bars);}root.add(mesh);return mesh;});
 const reviewBadges=cards.map((card,i)=>['Reviewing','Passed over','Selected'].map((text,j)=>{const badge=textPlane(text,`file-status-${i}-${j}`,1.6);badge.position.set(0,-1.08,.15);badge.material.color.set(j===1?'#a39b97':j===2?'#b9e9df':'#aecce2');badge.renderOrder=12;card.add(badge);return badge;}));
 const designBoard=texturePlane(designBoardTexture(),2.6,2.6,'design-direction');designBoard.renderOrder=5;root.add(designBoard);
 ...
 return {reviewBadges,setLanguage,reportFaces,dataFragments,root,body,robot,designBoard,page,crystal,cursor,projection,screenBody,scanLine,ambience,floor,scan,scanEdge,links,nodes,records,beam,screen,cards,product,reflection,heroLight,productCaption,orbit,commerce,details,commerceMetrics};
}
