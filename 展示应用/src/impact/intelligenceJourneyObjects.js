...
export function createJourneyObjects(manager){
 ...
 const texts=reviews;
 const reviewMap=reviewTexture(),purchaseMap=purchaseReviewTexture();
 const records=Array.from({length:texts.length},(_,i)=>{const text=texts[i%texts.length],mesh=textPlane(text,`record-${i}`,i===0?3.8:2.5);installTyping(mesh,text,mesh.material.map.image.getContext('2d'),i===0?3.8:2.5);const card=texturePlane(reviewMap,i===0?4.2:3.2,i===0?1.31:1,'feedback-frame');card.position.z=-.03;mesh.add(card);const purchase=texturePlane(purchaseMap,i===0?4.2:3.2,i===0?1.31:1,'purchase-review-frame');purchase.position.z=-.035;mesh.add(purchase);const faceTitle=textPlane(['Customer voice','Product design','Market signals','Colour trends','Usage occasions','Quality feedback'][i],'cube-face-label',i===0?3.7:2.8);faceTitle.position.set(0,.4,.015);mesh.add(faceTitle);mesh.traverse(part=>{if(part.material)part.material.side=THREE.FrontSide;});root.add(mesh);return mesh;});
