...
const reviewSlots=[[0,0],[-3.1,1.6],[0,2.15],[3.1,1.6],[-3.1,-1.7],[3.1,-1.7]];
// Each of the six source cards has one destination on a broad cube face.
const cardSeats=[[-.63,.62,1.48,0,0],[.63,-.62,1.48,0,0],[-1.48,.62,-.63,0,-Math.PI/2],[-1.48,-.62,.63,0,-Math.PI/2],[-.63,1.48,-.55,-Math.PI/2,0],[.63,1.48,.55,-Math.PI/2,0]];
export function intelligenceWorld(quality,manager){
 ...
 return {root,
  update({camera,target,aspect=16/9,intelligence=null,time=null}){
   ...
   o.body.rotation.set(lerp(.17,.025,f.crystal)+presencePose.float.x*.4,lerp(.45+.075*(t-8)*ramp(t,7,12),.12,f.crystal)-presencePose.turn*.7+presencePose.float.y*.6,lerp(-.04,-.02,f.crystal)+presencePose.float.z);o.body.rotation.y+=idleSway;o.body.position.y+=idleSway*.35;
   // Keep the remote data assembly and motion; the local mountain replaces its robot.
   o.crystal.morph(0,t);o.crystal.shell.scale.setScalar(1-ramp(t,25,30)*.75);
   alpha(o.crystal.shell,glassVisibility*.12);alpha(o.crystal.inner,0);alpha(o.crystal.edges,glassVisibility*.23);
   ...
   o.records.forEach((mesh,i)=>{
    ...
    if(t<34){
     ...
     s=lerp(i===0?1:.55,.27,localGather);a=onset*(1-ramp(t,25.5,30.5));
    ...
    mesh.position.set(x,y,z);mesh.scale.setScalar(s);if(t>=34)mesh.rotation.set(0,t>=85&&i>0?Math.sin(i)*.08*ramp(t,93,99):0,0);alpha(mesh,a*(count>0?1:0));alpha(mesh.children[0],a*framing);
   });
