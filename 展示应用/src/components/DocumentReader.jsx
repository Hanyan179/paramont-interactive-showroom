...
export function DocumentReader({document:doc,lang,onLanguage,onClose,onHome,initialSection,memory,onRemember}) {
  ...
  return <div className="document-scrim"><section ref={root} className={`document-reader ${large?'reading-large':''}`} role="dialog" aria-modal="true" aria-labelledby="reading-title" data-document={doc.id}>
    <header className="reading-toolbar">
      <button className="reading-return" onClick={()=>photo!==null?setPhoto(null):onClose()}><ArrowLeft/>{photo!==null?t('返回文档','Back to document'):t('返回场景','Back to scene')}</button>
